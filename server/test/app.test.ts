...
test('authentication, generated credentials, feature and balance contract', async () => {
  const f = await fixture(); try {
    ...
    const me = await f.app.inject({ method: 'GET', url: '/v1/me', headers: { authorization: `Bearer ${token}` } }); assert.equal(me.statusCode, 200); assert.equal(me.json().balance, 0); assert.equal(me.json().features.evaluate, true); assert.equal(me.json().features.videoSearch, true); assert.equal(me.json().features.commentReply, true); assert.equal(me.json().features.liveInteraction, true);
    const restricted = await f.create({ username: 'restricted', features: { videoSearch: false, commentReply: false, liveInteraction: false } }); const restrictedLogin = await f.app.inject({ method: 'POST', url: '/v1/auth/login', payload: { username: restricted.username, password: restricted.password, deviceId: 'pc-2', deviceName: '受限电脑' } }); const restrictedMe = await f.app.inject({ method: 'GET', url: '/v1/me', headers: { authorization: `Bearer ${restrictedLogin.json().token}` } }); assert.equal(restrictedMe.json().features.videoSearch, false); assert.equal(restrictedMe.json().features.commentReply, false); assert.equal(restrictedMe.json().features.liveInteraction, false);
...
test('workflow credit is bound to the run and only commits after server COMPLETED', async () => {
  const provider = await resultDecisionServer(); const f = await fixture({ provider: { baseUrl: provider.baseUrl, apiKey: 'test-key', model: 'test-model' } }); try {
    const workflow = await f.app.inject({ method: 'POST', url: '/v1/admin/workflows', headers: { authorization: `Bearer ${f.adminToken}` }, payload: { workflowId: 'credit.gated', version: 1, name: '扣费闸门', contract: { steps: ['done'] } } }); assert.equal(workflow.statusCode, 200);
    const user = await f.create({ username: 'credit-gated' }); await f.app.inject({ method: 'POST', url: `/v1/admin/users/${user.id}/credits`, headers: { authorization: `Bearer ${f.adminToken}` }, payload: { amount: 1, idempotencyKey: 'credit-gated-seed' } });
    const login = await f.app.inject({ method: 'POST', url: '/v1/auth/login', payload: { username: user.username, password: user.password, deviceId: 'pc', deviceName: 'A' } }); const token = login.json().token;
    const reserved = await f.app.inject({ method: 'POST', url: '/v1/credits/actions/reserve', headers: { authorization: `Bearer ${token}` }, payload: { actionKey: 'credit-gated-action', owner: 'workflow:credit.gated', amount: 1, metadata: { purpose: 'search' } } }); assert.equal(reserved.statusCode, 200);
    const actionId = reserved.json().action.id; (f.app as any).store.run('INSERT INTO workflow_plans(id,user_id,workflow_id,workflow_version,params_json,status,created_at,expires_at) VALUES(?,?,?,?,?,?,?,?)', 'credit-gated-plan', user.id, 'credit.gated', '1', '{}', 'issued', Date.now(), Date.now() + 60_000);
    const created = await f.app.inject({ method: 'POST', url: '/v1/workflow-runs', headers: { authorization: `Bearer ${token}` }, payload: { planId: 'credit-gated-plan', workflowId: 'credit.gated', version: 1, params: {}, creditActionId: actionId, idempotencyKey: 'credit-gated-run' } }); assert.equal(created.statusCode, 200, created.body); const runId = created.json().run.id;
    const early = await f.app.inject({ method: 'POST', url: `/v1/credits/actions/${actionId}/commit`, headers: { authorization: `Bearer ${token}` }, payload: {} }); assert.equal(early.statusCode, 409); assert.equal(early.json().code, 'CREDIT_ACTION_GATE');
    const started = await f.app.inject({ method: 'POST', url: `/v1/workflow-runs/${runId}/checkpoints`, headers: { authorization: `Bearer ${token}` }, payload: { status: 'RUNNING', expectedVersion: 0 } }); assert.equal(started.statusCode, 200);
    const done = await f.app.inject({ method: 'POST', url: `/v1/workflow-runs/${runId}/checkpoints`, headers: { authorization: `Bearer ${token}` }, payload: { status: 'COMPLETED', expectedVersion: 1 } }); assert.equal(done.statusCode, 200);
    const decision = await f.app.inject({ method: 'POST', url: `/v1/workflow-runs/${runId}/result-decision`, headers: { authorization: `Bearer ${token}` }, payload: { status: 'COMPLETED', summary: { confirmed: true }, idempotencyKey: 'credit-gated-decision' } }); assert.equal(decision.statusCode, 200);
    const ledger = await f.app.inject({ method: 'GET', url: '/v1/credits/ledger', headers: { authorization: `Bearer ${token}` } }); assert.equal(ledger.json().balance, 0); assert.equal((f.app as any).store.get('SELECT status FROM credit_actions WHERE id=?', actionId).status, 'committed');
  } finally { await f.close(); await new Promise<void>((resolve) => provider.server.close(() => resolve())); }
});
