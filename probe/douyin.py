...
def force_page_active(cdp):
    ...
    2026-09-19 真机发现：Chrome 窗口明明在前台（GetForegroundWindow 就是它、
    窗口没最小化），只要被别的窗口【遮挡】，document.visibilityState 仍是 "hidden"。
    此时抖音直播间的弹幕虚拟列表【一条都不渲染】——
    实测 30 秒里 WebSocket 一直在收帧，DOM 始终为空，采集结果 0 条，
    看起来和"直播间没人说话"一模一样（又一次静默错误）。

    这两条 CDP 命令可以解除它：
      · Page.setWebLifecycleState(active)  —— 把被冻结/降级的页面拉回 active
      · Emulation.setFocusEmulationEnabled —— 让页面认为自己在焦点上
    它们只影响【浏览器自己的调度与页面状态】，不触碰平台的任何风控或技术措施。
    """
    ok = []
...
def ensure_visible(cdp, log=None):
    ...
    if visibility_state(cdp) != "visible" and force_page_active(cdp):
        # 遮挡判定是 Windows 给的，CDP 的 bringToFront/activateTarget 改不了它；
        # 上面这两条命令能把页面本身拉回 active（真机实测：hidden -> visible，内容随即开始渲染）。
        time.sleep(0.8)
...
_DM_ENTRY_JS = (
    ...
    # 🔴 命中测试（2026-09-19，与直播间那套同一类坑）：
    #    "rect 非零 + 样式可见"不等于"这个坐标上点得到它" ——
    #    页面上可能另有一个元素盖在上面，点了等于没点，而且不报错。
    # 在按钮矩形里【采样若干点】，找一个"点得到它"的点。
    # 🔴 2026-09-19 真机：有两次失败不是按钮没找到，而是它的中心点被一个
    #    搜索框（INPUT.semi-input）盖住了 —— 照中心点去点就等于点了搜索框。
    #    改成采样 3x3 个点，命中即用；全都点不到才如实报 on_top:false。
    "function findPoint(e){try{var r=e.getBoundingClientRect();"
    " var xs=[0.5,0.2,0.8,0.35,0.65], ys=[0.5,0.35,0.65,0.2,0.8];"
    " for(var i=0;i<xs.length;i++){for(var j=0;j<ys.length;j++){"
    "  var x=Math.round(r.x+r.width*xs[i]),y=Math.round(r.y+r.height*ys[j]);"
    "  var h=document.elementFromPoint(x,y);"
    "  if(h&&(e===h||e.contains(h)||h.contains(e)))return {x:x,y:y,on_top:true};}}"
    ...
    " var pt=findPoint(n);"
    " cands.push({x:pt?pt.x:Math.round(r.x+r.width/2),y:pt?pt.y:Math.round(r.y+r.height/2),"
    "  on_top:!!pt,cls:String(n.className||'').slice(0,40),"
    "  cx:Math.round(r.x+r.width/2),cy:Math.round(r.y+r.height/2)});}"
    "if(!cands.length) return {found:false,blocked:false,reason:'dm_button_not_found'};"
    "var pick=cands[0];"
    "for(var j=0;j<cands.length;j++){if(cands[j].on_top){pick=cands[j];break;}}"
    "var covered=null;"
    "if(!pick.on_top){var e2=document.elementFromPoint(pick.cx,pick.cy);"
    " if(e2) covered={tag:e2.tagName,cls:String(e2.className||'').slice(0,50),"
    "  text:(e2.innerText||'').replace(/\\s+/g,' ').slice(0,30)};}"
    "return {found:true,blocked:false,x:pick.x,y:pick.y,on_top:pick.on_top,"
    " candidates:cands.length,covered_by:covered};"
    "})()"
