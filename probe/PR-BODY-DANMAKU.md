...
> **本轮更新（按审核意见重做）**：分支已**基于最新 `rewrite/v4-agent` 重建**（`0 落后 / 4 领先`，无冲突，CI 可正常调度），
> 并拆成 4 个干净提交；同时并入 **PR #11 的搜索终止态与游标池修复**（保留作者归属）。
> 原 PR #13 的分支落后基线 27 个提交、mergeable_state=dirty，因此 GitHub 连 CI 都不调度 —— 已由本 PR 取代。
