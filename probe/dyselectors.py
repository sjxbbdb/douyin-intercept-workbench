...
为什么叫 dyselectors 而不是 selectors（真机踩坑，别改回去）：
    本目录下的模块名会【顶掉同名的标准库模块】。叫 selectors.py 时，
    任何用到标准库 selectors 的代码都会炸 —— 实测报错：
        module 'selectors' has no attribute 'SelectSelector'
    （Python 3.13 的 http.server 就会踩到）。加 dy 前缀是成本最低的修法。
"""
