...
// 风控友好：两条回复之间的间隔与单次运行上限都可配置
const GAP_MIN_MS = Number(process.env.REPLY_GAP_MIN_MS || 10000);
const GAP_MAX_MS = Number(process.env.REPLY_GAP_MAX_MS || 14000);
const MAX_PER_RUN = Number(process.env.REPLY_MAX_PER_RUN || 0); // 0 = 不限
...
const BTN_NORM = "\\s\\[\\]，。,.!！?？:：;；、\\\"'“”‘’()\\[\\]{}<>《》【】（）\\-_=+~`@#$%^&*";
// 昵称/文本里常混入零宽与组合型不可见字符，比对前统一剔除
const INVIS_CLASS = "[\\u200B-\\u200F\\u202A-\\u202E\\u2060-\\u206F\\uFEFF\\u034F\\u00AD]";
// 公共片段：按评论文字定位评论项（昵称只作「优先项」，DOM 里昵称可能被截断或含不可见字符，不作硬性条件）
// 从原始评论文本里抽「最长的一段连续文字」作为 XPath 文本锚点：
// 表情标记（[流泪]）、标点、截断都不会影响它，用于归一化匹配失败时的兜底定位。
function xpathNeedleLiteral(rawText) {
  const runs = String(rawText || "").match(/[\u4e00-\u9fa5A-Za-z0-9]{4,}/g) || [];
  runs.sort((a, b) => b.length - a.length);
  const needle = runs[0] || String(rawText || "").trim().slice(0, 8);
  return JSON.stringify(needle);
}

function locateItemJs(qLiteral, nickLiteral, doScroll, needleLiteral) {
  return " var q=" + qLiteral + "; var n=" + nickLiteral + ";" +
    " var __INV=new RegExp(" + JSON.stringify(INVIS_CLASS) + ",'g');" +
    " var __BTAG=new RegExp(" + JSON.stringify('\\[[^\\]]*\\]') + ",'g');" +
    " q=(q||'').replace(__INV,'').toLowerCase(); n=(n||'').replace(__INV,'').toLowerCase();" +
    " var RE=new RegExp(" + JSON.stringify(BTN_NORM) + ",'g');" +
    " var __lists=Array.from(document.querySelectorAll('[data-e2e=\"comment-list\"]')); var __root=null; for(var __L=0;__L<__lists.length;__L++){ var __lr=__lists[__L].getBoundingClientRect(); if(__lr.width>0&&__lr.height>0){ __root=__lists[__L]; break; } } if(!__root) __root=document;" +
    " var __items=Array.from(__root.querySelectorAll('[data-e2e=\"comment-item\"]'));" +
    " var __hit=-1,__hitNick=-1,__norm='';" +
    " for(var __i=0;__i<__items.length;__i++){ __norm=(__items[__i].innerText||'').replace(__INV,'').replace(__BTAG,'').replace(RE,'').toLowerCase(); if(__norm.indexOf(q)<0) continue; if(__hit<0) __hit=__i; if(n.length>1 && __norm.indexOf(n)>=0){ __hitNick=__i; break; } }" +
    " var __idx=__hitNick>=0?__hitNick:__hit;" +
    " var it=__idx>=0?__items[__idx]:null;" +
    (needleLiteral ? " if(!it){ var __nd=" + needleLiteral + "; if(__nd && __nd.length>=3){ try{ var __rs=document.evaluate(\"//*[@data-e2e='comment-item']\", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null); var __hc=0; for(var __k=0; __k<__rs.snapshotLength; __k++){ var __node=__rs.snapshotItem(__k); if((__node.textContent||'').indexOf(__nd)>=0){ __hc++; it = (__hc===1) ? __node : null; } } }catch(e){} } }" : "") +
    " if(!it) return null;" +
    (doScroll ? " it.scrollIntoView({block:'center'});" : "");
}

// 评论项内取「回复」控件坐标：优先视口内的；取不到就把控件本身滚进视口再量
const REPLY_PICK_JS =
  " var els=Array.from(it.querySelectorAll('button,[role=\"button\"],a,div,span'));" +
  " var cand=els.filter(function(e){ var t=(e.innerText||e.textContent||'').replace(/\\s+/g,'').trim(); var r=e.getBoundingClientRect(), s=getComputedStyle(e); return t==='回复' && r.width>0 && r.height>0 && s.visibility!=='hidden' && s.display!=='none'; });" +
  " if(!cand.length) return null;" +
  " cand.sort(function(a,b){ var ar=a.getBoundingClientRect(),br=b.getBoundingClientRect(); var as=(a.tagName==='BUTTON'||a.getAttribute('role')==='button')?10:0; var bs=(b.tagName==='BUTTON'||b.getAttribute('role')==='button')?10:0; return (bs+br.width*br.height/1000)-(as+ar.width*ar.height/1000); });" +
  " var __inview=cand.filter(function(e){ var r=e.getBoundingClientRect(); return r.bottom>=0 && r.top<=innerHeight; });" +
  " var pick=__inview.length?__inview[0]:cand[0];" +
  " if(!__inview.length){ try{ pick.scrollIntoView({block:'center'}); }catch(e){} }" +
  " var rr=pick.getBoundingClientRect();" +
  " if(rr.width<=0||rr.height<=0||rr.top<0||rr.bottom>innerHeight) return null;" +
  " return {x:Math.round(rr.x+rr.width/2), y:Math.round(rr.y+rr.height/2), tag:pick.tagName, count:cand.length, hadToScroll:(__inview.length?0:1)};";

function btnCoordsExpression(q, n, needle) {
  return "JSON.stringify((function(){" + locateItemJs(JSON.stringify(q), JSON.stringify(n), true, needle) + REPLY_PICK_JS + " })())";
}

function replyBtnFindExpression(q, n, needle) {
  return "JSON.stringify((function(){" + locateItemJs(JSON.stringify(q), JSON.stringify(n), false, needle) + REPLY_PICK_JS + " })())";
}
// 兜底：不依赖几何位置，直接用 DOM click 点「回复」控件（抖音 React 会响应非可信 click）
function domClickReplyExpression(q, n, needle) {
  return "JSON.stringify((function(){" + locateItemJs(JSON.stringify(q), JSON.stringify(n), true, needle) +
    " if(!it) return null;" +
    " var els=Array.from(it.querySelectorAll('button,[role=\"button\"],a,div,span'));" +
    " var cand=els.filter(function(e){ var t=(e.innerText||e.textContent||'').replace(/\\s+/g,'').trim(); return t==='回复'; });" +
    " if(!cand.length){ cand=Array.from(it.querySelectorAll('*')).filter(function(e){ var t=(e.innerText||'').trim(); return t==='回复'; }); }" +
    " if(!cand.length) return { ok:false, reason:'no_reply_element' };" +
    " var pick=cand[0]; pick.click();" +
    " var r=pick.getBoundingClientRect();" +
    " return { ok:true, x:Math.round(r.x+r.width/2), y:Math.round(r.y+r.height/2), count:cand.length };" +
    " })())";
}

// 一次调用完成「找评论 → 滚入视口 → DOM 点击『回复』→ 检测内联编辑器」。
// 必须原子：分两次调用时，滚动过程中的虚拟列表会把刚渲染出来的评论回收掉，
// 于是表现为「等待循环说找到了，随后又说按钮找不到」。
function findClickExpression(q, n, needle) {
  return "JSON.stringify((function(){" + locateItemJs(JSON.stringify(q), JSON.stringify(n), true, needle) +
    " if(!it) return {found:false, count:(typeof __items!=='undefined'?__items.length:-1)};" +
    " var count=(typeof __items!=='undefined'?__items.length:-1);" +
    " var reps=Array.from(it.querySelectorAll('*')).filter(function(e){ return (e.innerText||'').trim()==='回复' && e.children.length===0; });" +
    " if(!reps.length) return {found:true, clicked:false, reason:'no_reply_element', count:count};" +
    " reps[0].click();" +
    " var ed=it.querySelector('[contenteditable=true]');" +
    " return {found:true, clicked:true, editor:!!ed, count:count};" +
    "})())";
}

async function domClickReply(cdp, q, n, needle) {
  const res = await sendT(cdp, "Runtime.evaluate", { expression: domClickReplyExpression(q, n, needle), returnByValue: true }, 10000).catch(() => null);
  try { return res && res.result && res.result.value ? JSON.parse(res.result.value) : null; } catch { return null; }
}

async function replyOne(cdp, item, comment, sameVideo) {
  ...
  const needleLiteral = xpathNeedleLiteral(comment.text); // XPath 文本兜底锚点
  ...
  // 同一个视频的多条评论复用同一个页面：只有页面不在该视频上时才重新导航。
  // 这样「13 条评论 = 13 次打开视频」变成「1 个视频 = 1 次打开」。
  const videoUrl = "https://www.douyin.com/video/" + item.awemeId;
  let needNavigate = !sameVideo;
  if (!needNavigate) {
    const current = await sendT(cdp, "Runtime.evaluate", { expression: "location.href", returnByValue: true }, 8000).catch(() => null);
    const href = current && current.result ? String(current.result.value || "") : "";
    if (href.indexOf(String(item.awemeId)) < 0) needNavigate = true;
  }
  if (needNavigate) {
    ...
    console.log("  step: 复用当前已打开的视频页（跳过重新加载）");
  }
  ...
  let found = false;
  let earlyClicked = false; // 找到评论时是否已用 DOM 点击激活「回复」（避免后续滚动把评论滚走）
  const scrollBottom = async () => {
    // 滚轮必须打在「可见评论区」上：普通视频在左侧(约900)，图文帖的评论是右侧小浮层(约1280)，
    // 固定打在 (900,500) 会导致图文帖浮层完全不滚动（只能看到前 ~12 条）。
    let wx = 900, wy = 500;
    try {
      const posRes = await sendT(cdp, "Runtime.evaluate", {
        expression: "JSON.stringify((function(){ var ls=Array.from(document.querySelectorAll('[data-e2e=comment-list]')); for(var i=0;i<ls.length;i++){ var r=ls[i].getBoundingClientRect(); if(r.width>0&&r.height>0) return {x:Math.round(r.x+r.width/2), y:Math.round(r.y+r.height/2)}; } return null; })())",
        returnByValue: true,
      }, 6000).catch(() => null);
      const pos = posRes && posRes.result && JSON.parse(posRes.result.value);
      if (pos && pos.x) { wx = pos.x; wy = pos.y; }
    } catch {}
    await sendT(cdp, "Input.dispatchMouseEvent", { type: "mouseWheel", deltaX: 0, deltaY: 2000, x: wx, y: wy }, 6000).catch(() => {});
  ...
  for (let round = 0; round < 24; round++) {
    ...
    expression: findClickExpression(targetText, targetNick, needleLiteral),
      returnByValue: true,
    ...
    try {
      ...
      if (st.found) {
        found = true;
        // 同一次调用里已经点过「回复」（虚拟列表不会在这中间回收节点）
        if (st.clicked) {
          earlyClicked = true;
          console.log("  step: 原子操作命中：找到评论并点击「回复」" + (st.editor ? "（编辑器已出现）" : ""));
        } else {
          console.log("  step: 找到评论但没找到「回复」控件:", st.reason || "");
        }
        break;
      }
    } catch {}
  ...
  if (!found) {
    ...
    return { ok: false, error: "comment_not_rendered (该评论在接口里有、但网页版评论区不渲染/已被删除，无法回复)" };
  }

  // 定位「回复」按钮：把「找到评论 -> 滚入视口 -> 取按钮坐标」放在同一次调用里同步完成，
  // 避免两次调用之间评论节点被虚拟列表回收，导致「评论找到了但按钮找不到」。失败时重试若干次。
  let bp = null;
  for (let attempt = 0; attempt < 6 && !bp && !earlyClicked; attempt++) {
    if (attempt > 0) {
    ...
    const merged = await sendT(cdp, "Runtime.evaluate", { expression: btnCoordsExpression(targetText, targetNick, needleLiteral), returnByValue: true }, 10000).catch(() => null);
    const mv = merged && merged.result && JSON.parse(merged.result.value);
    if (mv) { bp = mv; break; }
    // 合并调用已把评论滚入视口；等虚拟列表重渲染后再读一次按钮坐标（同步读会拿到 0×0）
    await sleep(1200);
    const again = await sendT(cdp, "Runtime.evaluate", { expression: replyBtnFindExpression(targetText, targetNick, needleLiteral), returnByValue: true }, 10000).catch(() => null);
    const av = again && again.result && JSON.parse(again.result.value);
    if (av) { bp = av; break; }
  }
  if (earlyClicked) {
    await sleep(2500); // 已经在找到评论的那一刻点过「回复」，这里只等内联编辑器渲染
  } else if (!bp) {
    // 几何定位失败（按钮在浮层外/被虚拟列表回收）时改用 DOM click，不再直接判失败
    const domClicked = await domClickReply(cdp, targetText, targetNick, needleLiteral);
    if (domClicked && domClicked.ok) {
      console.log("  step: 几何定位失败，改用 DOM 点击「回复」(候选 " + domClicked.count + ")");
      await sleep(2500);
    } else {
  ...
  for (let attempt = 0; attempt < 3 && !inp; attempt++) {
    if (attempt > 0) {
      // 先试 DOM 点击（不依赖几何位置）
      await domClickReply(cdp, targetText, targetNick, needleLiteral).catch(() => {});
      await sleep(1200);
      // retry the 回复 click (re-scroll to comment + click)
      const sc2 = await sendT(cdp, "Runtime.evaluate", {
        expression: "JSON.stringify((function(){ var q=" + qLiteral + "; var n=" + nickLiteral + "; var __INV=new RegExp(" + JSON.stringify(INVIS_CLASS) + ",'g'); var __BTAG=new RegExp(" + JSON.stringify('\\[[^\\]]*\\]') + ",'g'); q=(q||'').replace(__INV,'').replace(__BTAG,'').toLowerCase(); n=(n||'').replace(__INV,'').replace(__BTAG,'').toLowerCase(); var __lists=Array.from(document.querySelectorAll('[data-e2e=\"comment-list\"]')); var __root=null; for(var __L=0;__L<__lists.length;__L++){ var __lr=__lists[__L].getBoundingClientRect(); if(__lr.width>0&&__lr.height>0){ __root=__lists[__L]; break; } } if(!__root) __root=document; var items=Array.from(__root.querySelectorAll('[data-e2e=\"comment-item\"]')); for(var i=0;i<items.length;i++){ var txt=(items[i].innerText||'').replace(__INV,'').replace(__BTAG,''); var norm=txt.replace(/[\\s\\[\\]，。,.!！?？:：;；、\\\"'“”‘’()\\[\\]{}<>《》【】（）\\-_=+~`@#$%^&*]/g,'').toLowerCase(); if(norm.indexOf(q)>=0){ items[i].scrollIntoView({block:'center'}); return true; } } return false; })())",
        returnByValue: true,
      }, 10000);
      await sleep(1800);
      const btn2 = await sendT(cdp, "Runtime.evaluate", {
        expression: "JSON.stringify((function(){ var q=" + qLiteral + "; var n=" + nickLiteral + "; var __INV=new RegExp(" + JSON.stringify(INVIS_CLASS) + ",'g'); var __BTAG=new RegExp(" + JSON.stringify('\\[[^\\]]*\\]') + ",'g'); q=(q||'').replace(__INV,'').replace(__BTAG,'').toLowerCase(); n=(n||'').replace(__INV,'').replace(__BTAG,'').toLowerCase(); var __lists=Array.from(document.querySelectorAll('[data-e2e=\"comment-list\"]')); var __root=null; for(var __L=0;__L<__lists.length;__L++){ var __lr=__lists[__L].getBoundingClientRect(); if(__lr.width>0&&__lr.height>0){ __root=__lists[__L]; break; } } if(!__root) __root=document; var items=Array.from(__root.querySelectorAll('[data-e2e=\"comment-item\"]')); for(var i=0;i<items.length;i++){ var itemText=(items[i].innerText||'').replace(__INV,'').replace(__BTAG,''); var norm=itemText.replace(/[\\s\\[\\]，。,.!！?？:：;；、\\\"'“”‘’()\\[\\]{}<>《》【】（）\\-_=+~`@#$%^&*]/g,'').toLowerCase(); if(norm.indexOf(q)<0) continue; var els=Array.from(items[i].querySelectorAll('button,[role=\"button\"],a,div,span')); var cand=els.filter(function(e){ var t=(e.innerText||e.textContent||'').replace(/\\s+/g,'').trim(); var r=e.getBoundingClientRect(), s=getComputedStyle(e); return t==='回复' && r.width>0 && r.height>0 && r.bottom>=0 && r.top<=innerHeight && s.visibility!=='hidden' && s.display!=='none'; }); if(cand.length){ var r=cand[0].getBoundingClientRect(); return {x:Math.round(r.x+r.width/2), y:Math.round(r.y+r.height/2)}; } } return null; })())",
        returnByValue: true,
      }, 10000);
    ...
    const inpRes = await sendT(cdp, "Runtime.evaluate", {
      expression: "JSON.stringify((function(){ var q=" + qLiteral + "; var n=" + nickLiteral + "; var __INV=new RegExp(" + JSON.stringify(INVIS_CLASS) + ",'g'); var __BTAG=new RegExp(" + JSON.stringify('\\[[^\\]]*\\]') + ",'g'); q=(q||'').replace(__INV,'').replace(__BTAG,'').toLowerCase(); n=(n||'').replace(__INV,'').replace(__BTAG,'').toLowerCase(); var __lists=Array.from(document.querySelectorAll('[data-e2e=\"comment-list\"]')); var __root=null; for(var __L=0;__L<__lists.length;__L++){ var __lr=__lists[__L].getBoundingClientRect(); if(__lr.width>0&&__lr.height>0){ __root=__lists[__L]; break; } } if(!__root) __root=document; var items=Array.from(__root.querySelectorAll('[data-e2e=\"comment-item\"]')); for(var i=0;i<items.length;i++){ var t=(items[i].innerText||'').replace(__INV,'').replace(__BTAG,''); var norm=t.replace(/[\\s\\[\\]，。,.!！?？:：;；、\\\"'“”‘’()\\[\\]{}<>《》【】（）\\-_=+~`@#$%^&*]/g,'').toLowerCase(); if(norm.indexOf(q)>=0 && t.indexOf('回复中')>=0){ var ed=items[i].querySelector('[contenteditable=true]'); if(ed){ ed.focus(); var r=ed.getBoundingClientRect(); return {ok:true, x:Math.round(r.x+Math.min(100,Math.max(20,r.width/2))), y:Math.round(r.y+r.height/2)}; } } } return {ok:false}; })())",
      returnByValue: true,
    }, 10000);
  ...
  if (!netHits.find((h) => h.status === 200)) {
    ...
    const fb = await sendT(cdp, "Runtime.evaluate", {
      expression: "JSON.stringify((function(){ var q=" + qLiteral + "; var n=" + nickLiteral + "; var __INV=new RegExp(" + JSON.stringify(INVIS_CLASS) + ",'g'); var __BTAG=new RegExp(" + JSON.stringify('\\[[^\\]]*\\]') + ",'g'); q=(q||'').replace(__INV,'').replace(__BTAG,'').toLowerCase(); n=(n||'').replace(__INV,'').replace(__BTAG,'').toLowerCase(); var __lists=Array.from(document.querySelectorAll('[data-e2e=\"comment-list\"]')); var __root=null; for(var __L=0;__L<__lists.length;__L++){ var __lr=__lists[__L].getBoundingClientRect(); if(__lr.width>0&&__lr.height>0){ __root=__lists[__L]; break; } } if(!__root) __root=document; var items=Array.from(__root.querySelectorAll('[data-e2e=\"comment-item\"]')); for(var i=0;i<items.length;i++){ var t=(items[i].innerText||'').replace(__INV,'').replace(__BTAG,''); var norm=t.replace(/[\\s\\[\\]，。,.!！?？:：;；、\\\"'“”‘’()\\[\\]{}<>《》【】（）\\-_=+~`@#$%^&*]/g,'').toLowerCase(); if(norm.indexOf(q)>=0 && t.indexOf('回复中')>=0){ var svgs=items[i].querySelectorAll('svg'); for(var s=0;s<svgs.length;s++){ var p=svgs[s].querySelector('path[fill=\"#FE2C55\"]'); if(p){ var clickable=svgs[s].parentElement||svgs[s]; var r=clickable.getBoundingClientRect(); if(r.width>0&&r.height>0){ clickable.click(); return {clicked:true}; } } } } } return {clicked:false}; })())",
      returnByValue: true,
    }, 10000);
...
(async () => {
  ...
  let cdp = null;
  let lastAwemeId = null; // 同一个视频的多条评论复用同一个页面（一个视频只打开一次）
  let tabFails = 0;
  ...
  while (true) {
    ...
    try {
      // 池子反查失败时回退到队列自带评论内容（新抓取的评论可能还没进 filtered_comments.json）
      const commentText = lookupComment(item.commentId) || (item.commentText ? { text: item.commentText, user: item.user || item.nick } : null);
      const sameVideo = lastAwemeId === String(item.awemeId);
      const res = await replyOne(cdp, item, commentText, sameVideo);
      lastAwemeId = String(item.awemeId);
      if (res.ok) {
    ...
    const waitMs = GAP_MIN_MS + Math.floor(Math.random() * Math.max(1, GAP_MAX_MS - GAP_MIN_MS));
    console.log(new Date().toISOString().slice(11, 19), "waiting", waitMs / 1000, "s before next");
      processedThisRun++;
      if (MAX_PER_RUN > 0 && processedThisRun >= MAX_PER_RUN) { console.log("REPLY_RUN_LIMIT_REACHED 本次已处理", processedThisRun, "条，按配置停止（剩余任务保留在队列里）"); process.exit(0); }
    await sleep(waitMs);
  }
