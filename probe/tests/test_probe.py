...
class CommentBodyTextContractTests(unittest.TestCase):
    """评论正文提取的契约：纯数字正文必须能提出来。

    真机 2026-09-21：bodyText() 原来把【纯数字】一律当噪声跳过（本意是滤掉点赞数），
    于是正文就是「111」的评论提不出正文 -> rowMatches 必然失败 ->
    reply_comment_not_found。而「111 / 求带」这类短评论恰恰是截流最常见的命中目标。

    这里钉住生成出来的选择器 JS 本身；DOM 层的真实行为由 CI 里的
    ChromiumFixtureTests 用真实 Chrome 跑（本机沙箱起不了 Chrome）。
    """

    def test_numeric_fallback_is_present_and_used(self):
        import douyin
        js = douyin._row_helpers_js()
        self.assertIn("numBest", js, "纯数字兜底被删掉了：正文是「111」的评论会永远匹配不上")
        self.assertIn("return best||numBest;", js,
                      "兜底必须真的被返回：非数字候选优先，一个都没有时才用纯数字")
        self.assertIn("if(t.length>numBest.length)numBest=t;", js,
                      "纯数字候选要按最长取，避免被行内其它数字串（如计数）顶掉")

    def test_reply_at_pollution_stays_excluded(self):
        """「回复中」行里多出来的「回复@某人」比正文长，会把正文污染成 reply_row_mismatch。"""
        import douyin
        js = douyin._row_helpers_js()
        self.assertIn("if(/^回复@/.test(t))continue;", js)

    def test_timestamps_are_still_noise(self):
        """时间/地区这类噪声不能被兜底捞回来。"""
        import douyin
        js = douyin._row_helpers_js()
        self.assertIn("/^\\d+(秒|分钟|小时|天|周|月|年)前/.test(t))continue;", js)
