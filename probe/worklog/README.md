...
> ⚠️ **这份日志随分支 `docs/probe-worklog-keyword-chain` 提交，是【纯新增文件】——不修改基线任何既有文件。**
> 索引表里标注「直播间链路」的 evidence 文件属于姊妹分支 `feat/live-intercept-automation`，**不在本 PR 内**；
> 本 PR 实际包含：`worklog/README.md`、`worklog/PR-BODY.md`、`worklog/evidence/08` `09` `10`，
> 以及 `worklog/screenshots/04` `05` `06` 三张图。
>
> 📌 **代码部分不需要本 PR**：关键词搜视频 / 评论采集 / 四档关键词筛选 / 去重 这些能力，
> 基线 `rewrite/v4-agent` 的 `probe/` 里**已经有了**（本 PR 只补证据与踩坑记录）。
> 详见 `evidence/10-push-attempt.txt` 里记录的完整经过。
