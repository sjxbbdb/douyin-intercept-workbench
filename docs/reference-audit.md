...
旧服务端的积分和审计实现有较好的事务/幂等测试，但它是单仓库原型：默认配置、密钥生命周期、Linux 部署、备份恢复、租户隔离、升级回滚和可观测性需要按新授权端重新审查。回复生成成功时由服务端按生成服务计分，生成失败不扣分；发送结果仍单独审计，不能由桌面端自行扣分或自行改变上限。
...
6. 能力优先探索：视频搜索、评论采集与筛选、评论回复、直播互动和私信触达都可以开发；先在独立探针、专用 Chrome 或受控 sidecar 中取得与具体接入方式相匹配的证据，再按能力矩阵决定是否接入 desktop 和发行开关。官方 API 路径核验 scope/资格，浏览器路径核验专用账号、页面行为和平台结果，不能把官方 API 资格泛化为所有路径的前置条件。
7. 不实现逆向 API、Cookie/localStorage 导出、验证码/风控绕过、抓包重放或伪装成官方开放能力的路径。官方 API 返回错误、权限不足、频控和能力封禁都应变成明确的产品状态；sidecar 只能调用已审查的公开进程契约。

## 7. 本次验证记录

在克隆后的工作树运行 `npm test`，旧仓库入口结果为：L1 单元、L2 契约、L3 离线 DOM、L4 集成全部 PASS；最终一组集成输出为 60/60。入口明确写出 L5 真机验证和 L6 长稳不在测试范围内。

因此本报告的结论边界是：旧项目的授权、计费、CDP 协议和安全护栏有可复用实现依据；各抖音能力的当前线上可用性、开放平台资质（仅适用于官方 API 路径）和真实发送成功率仍需按接入方式由后续专门的真机验证完成。
