...
- 最终 Windows 包已完成隔离验收：portable 与 NSIS installer 均完成未授权空态、登录、退出、再次登录、`window.close()` 正常退出、同一 `userData` 重启恢复授权；正常退出时进程 `exitCode=0` 且 CDP 端口关闭。两种包都通过真实 `window.agentApi.probeSelectors({})` 走到随包 sidecar，并返回五项能力键。检查时移除了 `PATH` 与 `DOUYIN_PROBE_PYTHON`；installer 安装目录含 `resources/probe/probe-agent.exe`，真实 capabilities 返回 `protocolVersion=1`、无 stderr。portable SHA-256 为 `8304183E454FED51B357152C47141642EBFD5CB06B90BD86023ECC616C34CE56`（118,936,499 bytes），installer SHA-256 为 `C13E24D3957A83E2DF06A37FCE159A1D7813DD705687665CDAA69D10DDF7C185`（119,166,530 bytes），sidecar SHA-256 为 `CED43E4430A38E2CA47734AAB040C9EF6CDEA3B392CE8EFDB8005DF4DB535078`。
