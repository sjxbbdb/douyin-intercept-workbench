...
3. **安全策略由服务端权威下发，客户端只能调低不能调高**；上报的生效配置高于服务端策略即被拒绝（`POLICY_VIOLATION`）。
...
- 续期：**每次进程启动静默续期**，运行中剩余有效期 < 24 小时时续期；心跳响应若带新 `token_expires_ms` 亦须原子落盘。剩余有效期 > 0 → `POST /auth/refresh` 换新 token；已过期但未超 `token_refresh_window_ms`（30 天）→ 仍可用旧 token 调 refresh；超过 → 必须重新登录（`AUTH_TOKEN_EXPIRED`）。
- 轮换：续期签发新 token，旧 token 标记 `rotated`，在 `token_overlap_ms`（5 分钟）内仍可鉴权，覆盖在途请求。
- 失效：登出、被踢、账号停用、账号到期、**余额耗尽停机**、密码重置都立即失效。
...
## 3. 错误码总表
### 3.1 统一信封
失败：`{"ok":false,"code":"POLICY_VIOLATION","message":"客户端上报的生效配置高于服务端策略","detail":{"source_type":"dm","field":"daily_max","reported":60,"allowed":20,"policy_version":7},"server_time_ms":1758096060000}`
...
| `AUTH_FORBIDDEN` | 403 | 无权访问该资源 | 停机 + 上报日志 |
...
```
`stableStringify`：递归按 key 升序、无空格。校验失败 → 拒绝登录并提示网络不安全。**响应中的 `policy` 是权威安全策略**，客户端必须原样采纳（或调低），并在首次心跳中 ack（4.5）。
...
- **refresh**：鉴权 `Bearer <token>`（允许已过期 token）+ 签名，请求 `{"device_id":"...","client_version":"3.0.0","protocol_version":2}`；响应为登录响应中的 `token`/`token_expires_ms`/`sign_key`/`sign_key_expires_ms`/`privacy_salt`/`account`/`credit`/`policy`/`limits` + `prev_token_expires_ms`。**幂等**：同一旧 token 的并发 refresh 幂等（`token_overlap_ms` 内返回同一个新 token，不重复轮换）。**错误码** `AUTH_TOKEN_INVALID` `AUTH_TOKEN_EXPIRED` `AUTH_TOKEN_REVOKED` `AUTH_ACCOUNT_DISABLED` `AUTH_ACCOUNT_EXPIRED` `AUTH_SIGN_*`。
...
| `daily_used` / `pending_send_count` | object / int | 是 | 当日已发计数 `{"comment":40,"live_danmaku":12,"dm":1}` / 本地待上报明细条数 `12` |
| `instances` | array | 是 | `[{"instance_id":"dy_8848","engine_state":"running","daily_used_total":41}]` |
| `last_error_code` | string | 否 | 最近一次本地错误码 `"element_timeout"` |

```json
{"ok":true,"server_time_ms":1758096060000,"protocol_version":2,"ack_seq":1287,"state":"active","state_reason":null,
 "credit":{"balance_milli":61199000,"credit_per_reply_milli":1000,"updated_at_ms":1758096060000},
 "policy":{"policy_version":8,"account_tier":"warm_up","…":"结构同 4.1 的 policy 对象"},"policy_changed":true,"policy_ack_required":true,
 "daily_quota":{"comment":{"max":80,"used":40,"remaining":40},"live_danmaku":{"max":50,"used":12,"remaining":38},"dm":{"max":8,"used":1,"remaining":7}},
...
- `commands[]`：`{"type":"pause_engine","reason":"CREDIT_EXHAUSTED"}` / `resume_engine` / `{"type":"throttle","limits":{…},"reason":"POLICY_DAILY_CAP"}` / `{"type":"circuit_break","cooldown_until_ms":…,"reason":"failure_rate"}` / `reload_policy` / `{"type":"force_upgrade","url":"…"}`。客户端须在 **60 秒内**执行 `pause_engine` 与 `circuit_break`。
- **余额 ≤ 0 → HTTP 402 + `CREDIT_EXHAUSTED`**（标准错误信封，**带签名**，客户端必须验签后处理）。
- **错误码** `AUTH_*` `CREDIT_EXHAUSTED` `CREDIT_ACCOUNT_SUSPENDED` `POLICY_VIOLATION`(ack 上限高于服务端策略) `POLICY_TIER_UNKNOWN` `RATE_TOO_MANY_REQUESTS` `SERVER_*`｜**幂等**：**同一 `seq` 重复提交幂等**——返回与首次相同的响应体（缓存 10 分钟），不推进状态、不判重放；仅当 `seq < max_seq` 且不在缓存内才判 `AUTH_REPLAY`。
...
响应：`{"ok":true,"duplicate":false,"server_time_ms":…,"reconciliation":{"detail_confirmed":12,"reported_confirmed":12,"match":true,"audit_flags":[],"authoritative_source":"audit_sends"},"credit":{"balance_milli":61188000,"credit_per_reply_milli":1000},"commands":[]}`
...
| `failure_reason` | string\|null | 条件 | `verdict="failed"` 时必填（枚举见 7.5），其余必须为 `null` |
...
| `policy_exceeded` | 超出当日策略上限（明细仍留痕） | 否 |
| `unbilled_insufficient_credit` | 余额已耗尽，该条已发生但**不扣费、充值后也不补扣** | 否 |

- **逐条独立幂等**，键 `(account_id, send_id)`；重发/补报不重复扣费。
- **单向升级**：同 `send_id` 由 `sent_suspected`/`sent_confirmed_dom`/`failed` 升级为 `sent_confirmed` 且带平台证据 → 接受并**只计费一次**；降级或 `source_type`/`sent_at_ms` 不一致 → `AUDIT_SEND_CONFLICT`(409) 整条拒绝，保留首次记录。
...
- **错误码** `AUDIT_SEND_INVALID` `AUDIT_SEND_CONFLICT` `AUDIT_BATCH_TOO_LARGE` `POLICY_VIOLATION` `POLICY_VERSION_UNKNOWN` `POLICY_ACK_REQUIRED` `POLICY_TIER_UNKNOWN` `POLICY_DAILY_CAP_EXCEEDED`(200) `REPORT_PRIVACY_VIOLATION` `AUTH_*` `SERVER_*`｜**幂等**：由 `(account_id, send_id)` 唯一索引强制。
...
{"batch_id":"c-7d2e…41af","account_id":"acc_1001","device_id":"9f2c…0a93","seq":9,"protocol_version":2,
 "changes":[{"change_id":"ch-1a2b…9c0d","changed_at_ms":1758096000000,"source":"user","actor":"local_user","field_key":"limits.dm.daily_max","old_value":"20","new_value":"60","applied":false,"reject_code":"POLICY_VIOLATION","policy_version":7},
            {"change_id":"ch-2b3c…0d1e","changed_at_ms":1758096200000,"source":"server_policy","actor":"license_server","field_key":"limits.comment.daily_max","old_value":"30","new_value":"80","applied":true,"reject_code":null,"policy_version":8}]}
```
...
| 问题 | 数据来源 |
|---|---|
| 某次封号前该账号**实际生效的策略**是什么？ | `policy_ack_log(account_id, policy_version, policy_hash, applied_limits_json, first_ack_at_ms, last_seen_at_ms)` |
| 变化来自**服务端下发**还是**用户自己调低**？ | `audit_config_changes.source/actor` 与 `policy.version_history` 对比 |
| 用户是否**主动调高过**、系统是否**拒绝过**？ | `audit_config_changes` 中 `source=user` 且（`applied=false` 或 old→new 为调高）的行 |
| 封号前 N 小时具体发了什么、是否被风控过？ | `audit_sends`（仅哈希与判定，无原文），按 `source_type`/`verdict`/`sent_at_ms` 检索；`evidence.risk_control_signal` 与 `failure_reason` 聚合 |
...
```json
{"ok":true,"server_time_ms":1758096060000,"redeemed_milli":5000000,"balance_milli":66188000,"code_masked":"DSW-****-****-9A3F",
...
| `credits` | int | 套餐包含积分 = 可计费"平台确认成功回复"条数 × `credit_per_reply` 的积分值 |
...
## 5. HMAC 签名规范
### 5.1 算法与密钥
`HMAC-SHA256`，输出 64 位小写 hex。`sign_key` 由登录/续期响应下发（64 hex = 32 字节）；服务端存储 `sign_key = hmac_sha256(master_secret, account_id+"|"+device_id+"|"+key_epoch)`，只存 `key_epoch` 与 `sign_key_sha256`。`sign_key_expires_ms` 与 token 同寿命；客户端持久化到受保护目录，**不得写入日志**。
...
function settleSendBatch(acc, batch, nowMs) {
  ...
  try {
    for (const s of sortBySentAt(batch.sends)) {     // 严格按 sent_at_ms 升序
      ...
      const quota = dayQuota(acc, s.source_type, s.sent_at_ms)    // 3) 当日额度（UTC+8，按来源独立）
      const overLimit = quota.used >= quota.max
      if (overLimit) acc.audit_flags.push('policy_daily_cap_exceeded')
...
| 2 | 一批 20 条：12 条 `sent_confirmed`、5 条 `failed`、2 条 `sent_suspected`、1 条 `sent_confirmed_dom`（`bill_dom_confirmed=false`） | 扣 12 条积分；其余 8 条 `not_billable` 不扣费；明细全部留痕 |
...
| 5 | 离线 3 小时囤 1800 条，恢复后补报 | 受 `audit_batch_max=500` 限制分 4 批、按 `sent_at_ms` 升序串行上报，每批独立事务；跨批余额耗尽按 §6.5 处理 |
| 6 | 补报中某条 `send_id` 由首次 `sent_suspected` 升级为 `sent_confirmed` | 升级计费一次（§6.3）；同批不得出现两条同 `send_id` |
| 7 | 补报明细跨天（`sent_at_ms` 属不同自然日） | 按该条 `sent_at_ms` 所属自然日核算当日额度；超额部分 `policy_exceeded` 不扣费 |
| 8 | 一批 50 条，余额恰好够 10 条 | 前 10 条 `billed`；第 11 条起 `unbilled_insufficient_credit`；响应 `state="exhausted"` + `pause_engine` |
...
## 7. 字段字典（口径定义）
### 7.1 三来源枚举
`comment`（视频评论区）、`live_danmaku`（直播间弹幕）、`dm`（陌生人私信）。三者**独立限额、独立统计**；上报 `sources` 的三个键必须全部存在，无数据填全 0 对象。

### 7.2 `hits` / `leads_new` / `skipped` / `reply_attempts`
...
- **跳过 `skipped`**：命中但**没有发起回复**的条数（黑名单、当日额度已满、频控、该用户已回复过）；`reply_attempts + skipped ≤ hits`。
- **回复尝试 `reply_attempts`**：真正向平台发起发送动作的次数，每个都必须对应一条 `send_id` 明细。**`skipped` 与 `reply_attempts` 都不计费，只有成功的那部分计费。**

### 7.3 发送判定：四级口径（**本契约最关键的口径**）
...
- 约束 `sent_confirmed + sent_confirmed_dom + sent_suspected + failed = reply_attempts`（每个来源分别成立）。
- **禁止**把 `sent_suspected` 或 `sent_confirmed_dom` 计入 `sent_confirmed`。历史遗留的"编辑器消失即判成功"必须改写为 `sent_suspected` 并在上报中走**不同字段**，不得混为一谈。空响应（无 `platform_status_code`）一律 `failed` + `risk_control_signal="empty_response"`，不得标记成功。
- 四个字段在聚合上报中各自独立，不设合并字段，避免下游误用。

### 7.4 去重用户数 `unique_users`
...
| 分来源明细 | `send_log` 按 `source_type` 聚合的 `reply_attempts/sent_confirmed/failed` |
| 失败原因分布 | `failure_reasons` 聚合 |
| 积分消耗（日/周/月） | `credit_ledger` 按 `settled_at_ms` + `stats_tz_offset_minutes` 聚合，**客户端不得自行计算** |
| 近 7 日趋势 | 按自然日聚合 `leads_new/sent_confirmed/usage_milli` |
| 策略合规视图 | `daily_quota` 使用率、`policy_exceeded` 条数、`audit_config_changes` 中被拒的调高尝试 |

### 7.8 产品文案字段 `quota_notice`
...
| 字段语义变更 | 禁止 | 必须新增字段名，旧字段保留 |
| 删除/重命名字段 | 禁止 | 先标 `deprecated_since`，**至少保留 90 天且跨 3 个客户端版本**，双写期内同时返回新旧字段 |
...
- **策略版本未知**（上报了服务端无记录的 `policy_version`）→ `POLICY_VERSION_UNKNOWN`(409)，客户端拉 `GET /policy/current` 对齐后重报；该批不落账不计费。
...
6. `POST /heartbeat`（`seq=1`、`applied_policy_version=7`、`applied_limits={...}`）→ 服务端建立 `policy_ack_log{first_ack_at_ms, applied_limits_json}`，返回 `{state:"active", policy_changed:false, daily_quota, commands:[]}`。
