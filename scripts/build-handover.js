...
const PKG_NAME = 'Agent工作流平台-开发交接包'
...
function layOut(srcSnapshotRoot, pkgRoot, stats) {
  ...
  const rootItems = [
    ...
    'client', 'license-server', 'shared', 'packs',
...
| \`02-定位与需求\` | **项目章程（定位锚点）**、平台需求规格（①平台层验收依据）、抖音工作流包需求规格（②领域层） |
...
| \`client/\` \`license-server/\` \`shared/\` \`packs/\` \`test/\` \`scripts/\` | **全部源码**（在包根，因为 require 路径决定它们必须在这里） |
...
**尚未开始**（依 \`05-平台机制与迁移/D-重构迁移与验收.md\` 的 P10–P14）：
\`client/agent/\`（Agent 运行时）、\`client/tools/\`（工具层）、\`client/packs/\`（包加载器）、
\`packs/douyin-intercept/\`（把现有抖音能力抽成包）、\`action_kind\` 计费契约扩展。
...
3. **审计必须记录真实生效的策略值。** 只上传哈希与计数，
   **绝不上传**评论原文、回复原文、\`sec_uid\`。
   🆕 Agent 参与决策后，还须记录 \`pack_id\` / \`prompt_version\` / \`model.version\`。
...
4. 读 \`05-平台机制与迁移/D-重构迁移与验收.md\`，按 **P9 → P14** 执行。
   P9（文档与契约）与 P10（依赖与框架落地）已完成，从 **P11（工作流包机制）** 开始。
5. **要一个真实测试账号**，跑通真机验证门
   （\`02-定位与需求/抖音工作流包需求规格.md\` §七 G-1~G-8）。
   在那之前，任何"采集/回复已经能用"的说法都是不成立的。
6. 真机验证通过后，再做界面精修与 72 小时长稳测试。

> ### ⚠️ 最容易被做错的一件事
>
> **不要把"加一个新领域"实现成"改核心代码"。**
> 如果做第二个领域时改了 \`client/agent/\` 或 \`client/tools/\`，
> 说明工作流包抽象是错的 —— 应当回到
> \`05-平台机制与迁移/C-工作流包契约.md\` 重新对齐，而不是继续加 \`if\`。
