...
- 产品围绕抖音截流 Agent 展开。视频搜索、评论采集与筛选、评论回复、直播互动和私信触达都可以作为开发能力；每项能力必须由证据决定是否进入发行开关。
- Linux `server/` 是账号、授权、功能开关、积分、审计和策略的唯一权威。
- Windows `desktop/` 是可分发客户端：工作台账号由授权端提供，抖音账号由商家在专用窗口中手动登录。
- 桌面端由 Electron UI、主进程授权/任务编排、商家手动登录的专用 Chrome 和随包分发的 Python sidecar 组成；旧 Electron DOM 适配器保留作开发诊断与离线 fixture。官方开放 API 若被采用，单独记录 scope/资格；所有适配器正式接线前都必须记录来源、协议、失败状态和验证状态。
...
- 新增平台能力必须同时提交：官方文档或真机证据、能力矩阵、失败状态、脱敏测试、未验证边界和发行开关条件。
