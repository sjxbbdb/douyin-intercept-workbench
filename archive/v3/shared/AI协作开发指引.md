...
| 问题 | 查哪里 |
|---|---|
| **这个项目到底是什么？"专用"和"专业"各指什么？** | `docs/项目章程.md`（**定位锚点，与之冲突一律以它为准**） |
| **业务规则能不能写死在代码里？** | 不能。话术、判据、报价策略由**商家配置 → 服务端下发**（`docs/平台需求规格.md` FR-P6） |
| **哪些能配、哪些绝对不能配？** | 业务规则商家随便改；**安全数值只能厂商定、商家只能调低**（`docs/项目章程.md` §3） |
| 这个渠道每天最多发几条？最小间隔多少？ | `shared/protocol.md` §4.6 `tier_table`（**唯一来源，禁止硬编码**） |
| 什么情况算"发送成功"、可以计费？ | `shared/protocol.md` §6.1 计费资格（五条必须**同时**成立）+ §7.2 四级口径 |
| 客户端能不能把上限调高？ | 不能。`shared/protocol.md` §4.6「客户端只能更保守」的四个方向 |
| 相似度阈值是"超过拒绝"还是"低于拒绝"？ | `shared/protocol.md` §4.6 `content_similarity_max_semantics`：**超过即拒绝** |
| 某个错误码客户端应该怎么做？ | `shared/protocol.md` §3.2 错误码表「客户端动作」列 |
| 这个字段能不能上报？ | `shared/protocol.md` §7.5 隐私边界（禁用字段出现即整条被拒） |
| `failure_reason` 有哪些合法值？ | `shared/protocol.md` §7.4 闭集（未知键会被拒绝） |
| `platform_endpoint` 能写完整 URL 吗？ | 不能，闭集白名单只有 `comment/publish`、`comment/reply`、`im/send`、`live/comment/send` |

### 1.4 ⚠️ 定位类词汇：先问清楚，再翻译成架构

> ### **不要把用户说的一个词直接翻译成一个架构。这是本项目已经犯过两次的错误。**

| 用户原话 | 我读成 | 实际 | 代价 |
|---|---|---|---|
| "要做 agent" | 加一层 agent（默认关闭、界面为主入口） | **Agent 是承载形态**，默认开启 | 撤回三个默认值 + 全套文档 |
| "专业分化程度较高" | 要支持多个行业 → 设计了插件机制 | **"专用"修饰的是业务领域，不是技术能力** | 撤回一整套插件机制与三份文档 |

**两次错在同一个地方：注意力在"技术上能不能扩展"，而不在"用户到底要什么"。**

**强制流程**——听到下列词时，**必须先问一个澄清问题，不得直接开始设计**：

| 词汇 | 必须问清 |
|---|---|
| 专用 / 专门 / 专业 | 修饰的是**业务领域**还是**技术能力**？ |
| 平台 / 框架 / 通用 | 是要"支持多个领域"，还是"把一件事做到位"？ |
| 智能 / 自适应 | 是"模型判断"，还是"更好的规则"？ |
| 可扩展 / 灵活 | 现在就有具体需求，还是为将来预留？（**为假设需求付出的抽象成本是确定的，收益是想象的**） |

> **更实用的判据**：如果某个设计是为了"将来可能要支持 X"，
> 先问"现在有客户要 X 吗"。没有，就不做——
> **将来真的需要时，先看实际差异再抽象，比现在凭想象设计一套通用语言便宜得多。**

> **另一条推论（值得记住）**：
> 两次修正的行动成本之所以还能接受，是因为每次都只改了**文档与默认值**，
> 不需要拆掉代码重写。这不是运气——是因为代码被组织成了
> "机制（几乎不变）+ 业务（可配）"，定位变了不用动 `core/`、`platform/`、`license-server/`。
