...
| **C-8** | 文档引用的既有规范需回读 | 本文档引用了 `shared/开发规范.md`、`测试策略.md`、`安全与合规要求.md`、`已知陷阱与平台知识.md`、`AI协作开发指引.md` | 这些文档与本文件**同期编写**，可能存在口径细节差异（尤其**审计字段定义**与**测试分层**两处） | 开工前请**回读这六份文档**，确认：① 审计字段与本文 §7.2 一致；② 评估集在 `shared/测试策略.md` 中的分层定位与本文 §一 一致；③ `docs/架构说明.md` §六 的层间禁止事项与本文 §3.1 的四处 adapters 出口一致。**发现冲突以 `docs/平台需求规格.md` 与 `shared/protocol.md` 为准** |
...
| `docs/项目章程.md` | 项目定位锚点、三条红线的平台化表述 |
