...
>
> **前置条件（不可跳过）**：方案 A 的底座已完成。详见 `README-DEV.md` §六 路径 3。
>
> **核心设计的唯一定稿来源是 `plans/B-工具契约与粒度设计.md`。** 本文档只做展开与实现指导，
> **不新增、不删除、不修改任何工具语义**。两者冲突时以 `plans/B-工具契约与粒度设计.md` 为准。
>
> 配套文档：`plans/B-工具契约与粒度设计.md`（契约）· `plans/B-评估集与成本模型.md`（评估与成本）· `plans/B-与方案A的差异说明.md`（增量改动）
...
商家的使用方式从"填表单、点按钮"变为"用自然语言下指令"，由 Agent 编排一组**粗粒度业务工具**完成
查询、分析、审批、汇报。**所有真实发送动作仍由既有代码执行，走完整安全护栏。**
...
四条设计理由（引自 `plans/B-工具契约与粒度设计.md` §一）：

1. **硬规则不能交给概率模型。** 红线 1 的"观察期禁发""日上限""最小间隔"是硬约束；护栏必须是代码里的 `if`。
2. **成本可控。** 批处理一次决策处理 N 条，而不是每条一次推理。
3. **可审计。** 决策输入是有限的候选集，输出是"批准 + 选择"，可完整落盘。
4. **降级安全。** Agent 不可用时系统退化为方案 A 继续工作。
...
| 组成 | 方案 A | 方案 B |
|---|---|---|
| 授权中心、契约、计费、审计、安全策略下发 | ✅ 完整实现 | ✅ **原样复用，零改动** |
| `client/core/` `client/platform/` `client/license/` | ✅ | ✅ **原样复用** |
| `client/adapters/` | 内部流水线 | 🔧 **暴露"预筛候选"能力，供工具包装** |
| `client/safety/` | 护栏 | 🔧 **新增拦截 Agent 越权意图的一层** |
| `client/ui/` 图形界面 | ✅ | ✅ **保留，作为降级路径** |
| 对话界面 + Agent 运行时 + 工具层 | — | 🆕 **新增** |

> **⚠️ B 不能跳过 A 的底座。** 三条硬性理由：
>
> 1. Agent 调用的全部工具，最终落到 `adapters/` 与 `safety/`；这些不存在，工具层无物可包。
> 2. 计费与审计口径（红线 2、红线 3）在 A 中已由服务端契约固化；B 的每次决策要挂到同一套审计上，
>    没有 A 就没有可挂载的审计通道。
> 3. **A 才能回答"裸 CDP 驱动抖音这条路走不走得通"**（P2 验证门 G-1~G-5）。B 用的是同一套 CDP 驱动，
>    A 走不通，B 一样走不通，且 B 还多了一层成本。

### 1.4 适用与不适用场景

**适用**：

- 已跑通方案 A，且有数据表明关键词规则召回率不足（真实评论中"规则抓不住的意图"占比高）
- 商家愿意且能够接受"Agent 审批 + 人工可介入"的交互形态
- 定价空间足以覆盖 token 成本（按批处理设计后，见 `plans/B-评估集与成本模型.md` §五）
- 团队具备 LLM 应用经验，能承担评估集与成本监控的额外工作量

**不适用**：

- 规则召回率已足够（先跑一个月 A，用导出数据判断，不要凭感觉）
- 商家对"模型参与决策"有合规或心理阻力
- 单价空间无法覆盖 token 成本
- P2 真机验证门未通过
...
║  Agent 层（🆕 仅方案 B 有）                                                ║
║                                                                          ║
║   client/agent/runtime.js      循环：接收指令 → 规划 → 调工具 → 观察 → 汇报  ║
...
║   client/tools/query-leads.js         T-01                                ║
║   client/tools/get-stats.js           T-02                                ║
║   client/tools/get-account-health.js  T-03                                ║
║   client/tools/analyze-comments.js    T-04                                ║
║   client/tools/draft-replies.js       T-05                                ║
║   client/tools/review-batch.js        T-06                                ║
║   client/tools/approve-batch.js       T-07                                ║
║   client/tools/explain-failure.js     T-08                                ║
║   client/tools/validate.js      ⚠️ 参数校验（不信任模型传参）                ║
║   client/tools/audit.js         ⚠️ 每次工具调用落审计                        ║
║                                                                          ║
║   ⚠️ 工具层是「包装」，不是「重新实现」：内部一律调用既有 adapters/safety      ║
╚═════════════════════════════════╤════════════════════════════════════════╝
                                  │ 进程内调用（不经 HTTP、不经 IPC 转发）
╔═════════════════════════════════▼════════════════════════════════════════╗
...
║   client/core/      cdp.js（唯一一份）· browser-host.js · ipc.js          ║
...
```
┌──────┐        ┌───────────┐        ┌──────────┐        ┌──────────────┐        ┌────────┐
│ 用户 │        │ 对话界面  │        │ Agent    │        │ 工具层        │        │ 底座   │
│      │        │ (client/  │        │ runtime  │        │ (client/     │        │adapters│
│      │        │  ui/)     │        │          │        │  tools/)     │        │safety  │
└──┬───┘        └─────┬─────┘        └────┬─────┘        └──────┬───────┘        └───┬────┘
   │  "…别超过 10 个"  │                   │                     │                    │
   ├─────────────────►│  POST /api/agent/chat                    │                    │
   ...
   │                  │                   │  {…}                │  true, remaining…} │
   │                  │                   │                     │                    │
   │                  │                   │ ③ 再推理 → T-01 query_leads(intent:"price_inquiry")
   │                  │                   ├────────────────────►│                    │
   │                  │                   │                     ├───────────────────►│
   │                  │                   │                     │  只读线索库（脱敏） │
   │                  │                   │                     │◄───────────────────┤
   │                  │                   │◄────────────────────┤ {total:17, leads[]}│
   │                  │                   │                     │                    │
   │                  │                   │ ④ 再推理 → T-06 review_batch(max_items:10)
   │                  │                   ├────────────────────►│                    │
   │                  │                   │                     │ ⚠️ 代码预筛：       │
   │                  │                   │                     │  · guard 额度/间隔  │
   │                  │                   │                     │  · similarity 去重  │
   ...
   │                  │                   │                     │                    │
   │                  │                   │ ⑤ 模型决策（批处理，一次推理覆盖 N 条）        │
   │                  │                   │    approve / reject / skip + draft_id 选择    │
   │                  │                   │                     │                    │
   ...
   │                  │                   │                     │  queued_send_ids}  │
   │                  │                   │                     │                    │
   │                  │                   │ ⑦ 落盘决策记录 decision_id（含 model/prompt/  │
   │                  │                   │    policy_version/usage/latency_ms）          │
   │                  │                   │                     │                    │
   │                  │◄──────────────────┤ ⑧ 自然语言汇报       │                    │
   │◄─────────────────┤  "看了南京 17 条询价，挑了 7 条回复（已发出），3 条是投诉或      │
...
| 层 | 输入 | 输出 | 明确不做什么 |
|---|---|---|---|
| **对话界面** | 用户自然语言 | HTTP 请求 → `client/host/api.js` | 不直连模型，不持有 API Key |
| **Agent 运行时** | 用户指令 + 落盘状态 + 工具返回 | 工具调用请求 / 汇报文本 | **不产出发送动作**，不碰 CDP，不碰盘（经 store） |
...
2. **工具 → 底座** 必须经 `client/host/store.js` 读状态（可写），**禁止工具自己 `fs.writeFileSync`**
   （违反单写者约束，见 `shared/术语与选型基准.md` §3.4）。
...
> **完整契约见 `plans/B-工具契约与粒度设计.md` §三。本节只讲"怎么实现"，不重复契约内容。**

### 3.1 工具是包装既有 adapters，不是重新实现

```
❌ 错误做法（会造出第二套 CDP 逻辑）：
   client/tools/review-batch.js 里自己写 Runtime.evaluate 去页面上读评论

✅ 正确做法（工具只做三件事：取数 → 组织 → 校验）：
   client/tools/review-batch.js
     → 调 client/adapters/collect.js 暴露的 listPendingCandidates()
     → 调 client/safety/guard.js 的 checkSendable()（额度/间隔/时段/观察期）
     → 调 client/safety/circuit.js 的 isOpen()
     → 调 client/safety/similarity.js 的 checkContent()
     → 组织成契约定义的 batch 结构返回
```

**为支持工具，`client/adapters/` 需要暴露的读取型能力**（方案 A 中是流水线内部步骤）：

| 能力 | 建议出口 | 用途 | 归属工具 |
|---|---|---|---|
| `listPendingCandidates({source_type, max_items})` | `adapters/collect.js` | 返回已过规则筛选、待审批的候选线索 | T-06 |
...
> ⚠️ **这些出口必须是"新增读取型方法"，不是把 adapters 拆开重写。** 方案 A 的流水线逻辑保持原样，
> 只是多一个入口把中间结果暴露出来。改动面见 `plans/B-与方案A的差异说明.md` §三。

### 3.2 工具注册与 schema 定义

`client/tools/registry.js` 是**工具清单的唯一来源**。列表本身必须与
`plans/B-工具契约与粒度设计.md` §三 逐条对应，不多不少正好 8 个（T-01~T-08）。

```js
// client/tools/registry.js  —— 纯 JavaScript（CommonJS），无 TypeScript
'use strict'
...
const TOOLS = [
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        instance_id: { type: 'string', description: '账号实例 ID，省略则用当前实例' },
      },
...
- 依赖白名单只有 `ws`，**不能引入 `ajv` 之类的校验库**。用约 80 行手写校验器
  （`client/tools/validate.js`），只支持本项目用到的子集：`type`/`required`/`enum`/`minimum`/`maximum`/
  `maxLength`/`items`/`additionalProperties`。
- `maximum: 'SERVER_POLICY'` 是占位标记，实现时替换为运行时从 `policy` 读取的值。
...
function validateArgs(schema, args) {
  ...
  for (const [name, spec] of Object.entries(schema.properties || {})) {
    ...
    if (spec.enum && !spec.enum.includes(v)) errors.push(`${name} 不在闭集枚举内: ${JSON.stringify(v)}`)
...
| 2 | `max_items` 超过服务端策略上限 → **截断到上限**（不报错，但要记审计） | 按上限执行 |
| 3 | `analyze_comments.lead_ids` 长度 > 50 → 拒绝 | 提示分批 |
...
function recordToolCall({ tool, argsDigest, result, code, latencyMs, promptVersion }) {
...
| 审计项 | 记录内容 | 理由 |
|---|---|---|
| 工具名 + 参数摘要 | 参数白名单字段的取值/哈希，**不外传原文** | 可复现"Agent 当时传了什么" |
| 结果码 | `ok` 或阻断码（§7.4 闭集） | 可统计"Agent 被拦了多少次" |
| 耗时 | `latency_ms` | 定位是工具慢还是模型慢 |
| `prompt_version` | 当时生效的提示词版本 | 与决策记录同源，保证可复现 |

**双写要求**：本地 `tool_audit.json` 保留全量（含本地可读摘要）；服务端只上报**计数与哈希**
（红线 3 隐私边界：只上传哈希与计数，不上传原文）。
上报通道**复用既有** `POST /api/v1/usage/report`（计数类）与 `POST /api/v1/audit/config-changes`（策略相关），
**不新增服务端接口**。详见 §7.2。
...
│       │                                                             │
│  ② 重建上下文 ⚠️ 从落盘状态重建（agent_task.json / agent_batch.json / │
│       │          decision_log.jsonl），不依赖对话历史                 │
│       │                                                             │
│  ③ 规划       组装 messages = [system(prompt_version), 状态摘要, 指令] │
│       │       → model-client.chat()                                 │
│       │                                                             │
│  ④ 判定      模型返回 tool_call？                                     │
│       │        ├─ 是 → ⑤                                            │
...
│       │                                                             │
│  ⑥ 观察      把工具结果（已裁剪为最小必要字段）追加进 messages          │
│       │       round++ ；若 round > MAX_ROUNDS → ⑦                    │
│       │                                                             │
│  ⑦ 回到 ③                                                            │
│       │                                                             │
...
| `SOFT_WARN_ROUNDS` | 8 | 到 8 轮时在系统消息中注入"请收敛，尽快给出结论" |
| `MAX_WALL_CLOCK_MS` | 180000（3 分钟） | 单次交互总墙钟上限（用 `process.hrtime.bigint()` 计算，不用墙钟时间） |
| `MAX_TOKENS_PER_INTERACTION` | 由预算配置决定 | 见 §8.4；耗尽即停止并汇报 |
...
**超限时的汇报要求**：必须明确告知用户"没做完"，并给出已完成的进度与建议，**不得静默截断**。
例如："我只完成了 8 条中的 6 条审批，因为单次处理轮次已达上限。已批准并投递 6 条，
剩余 2 条可以下次继续说'继续'"。
...
| 场景 | 行为 |
|---|---|
| 模型请求超时（无响应） | 退避重试 1 次（2 秒）；仍失败 → 汇报"模型服务无响应"，**不执行任何未经审批的发送** |
| 单个工具执行超时 | 工具内部超时（各 adapters 已有超时）→ 返回结构化错误给模型，模型可换策略或汇报 |
| 用户点"停止" | `POST /api/agent/stop` → 置中断标志；**当前工具调用跑完即停**（不中断 CDP 操作中途，避免留下半完成状态） |
| 进程崩溃 / 断电 | 状态已落盘：`agent_task.json` 里的 `status` 保持"进行中"，重启后提示"上次任务未完成，是否继续" |
| Agent 正在审批时用户点了急停 | 急停优先级最高：`approve_batch` 的二次校验会因 `POLICY_SENDING_DISABLED` 全部 `blocked`；Agent 汇报"发送已被急停关闭" |

> ⚠️ **Agent 没有急停工具**（`plans/B-工具契约与粒度设计.md` §二 反模式清单）。
> 急停是人在界面上的动作，通过既有 UI 按钮与 `client/host/api.js` 触发。

### 4.4 错误处理：工具返回错误时 Agent 应该怎么做

在**系统提示词**中明确约定（见 §5.2），并在**代码**中把错误结构化返回：

| 工具错误类型 | 期望 Agent 行为 | 代码层的保证 |
|---|---|---|
| 参数校验失败（如 `max_items` 超上限） | 用合法参数重试一次 | 工具层返回 `{ok:false, code:'INVALID_ARGS', hint:'max_items 上限为 N'}` |
| `AGENT_BATCH_EXPIRED` | **不重试**，重新调 `review_batch` 取新批次 | 工具层拒绝过期批次，返回新批次 ID 提示 |
| `POLICY_SENDING_DISABLED` | **立即停止**，向用户解释处于观察期/已停发 | `review_batch` 直接返回空批次；`approve_batch` 全部 `blocked` |
| `POLICY_CIRCUIT_OPEN` | **立即停止**，告知熔断原因与恢复时间 | `get_account_health().circuit` 有 `reason` 与 `cooldown_until_ms` |
...
**代码侧的硬性保证**：无论模型怎么"想"，工具的返回值只可能来自上述闭集；
`approve_batch` 的 `blocked[]` 是**服务端/主进程给出的最终事实**，Agent 无法通过多试几次改变它。

### 4.5 模型接入抽象

**目标**：为将来换模型留口，且**不引入白名单外的依赖**（`AGENTS.md` §2.14）。
...
async function chat({ messages, tools, temperature, maxTokens, timeoutMs }) {
  ...
  return {
    ...
    usage: {
      // ⚠️ 两家厂商的字段命名不同，统一在这里归一化
      prompt_tokens: adapter.extractUsage(raw).input,
      completion_tokens: adapter.extractUsage(raw).output,
    },
...
const ADAPTERS = {
  // 每个 adapter 只做三件事：拼请求体、发 HTTPS、把响应映射到统一结构
  // 具体厂商路径与字段名按实际接入的 API 文档填写，不要写死在业务代码里
}
...
| 字段 | 说明 |
|---|---|
| `enabled` | Agent 总开关。**默认 `false`**，需商家在设置中显式开启（与 FR-2.5「默认半自动」一致，见 §12 冲突项 C-1） |
| `provider` / `base_url` / `model` | 接入目标，可换 |
| `api_key` | ⚠️ 存本地受保护文件，**不得写入日志、不得上传、不得进入审计** |
| `temperature` | 建议 **0.2**（决策类任务要稳定，不要创造性） |
| `max_tokens_per_interaction` | 单次交互预算，见 §8.4 |
| `prompt_version` | 当前生效的提示词版本（§5.3） |
...
```
        ...
        ▼                           ▼                           ▼
┌───────────────────┐   ┌───────────────────┐   ┌───────────────────────────┐
│ L1 模型服务不可用  │   │ L2 成本超预算      │   │ L3 Agent 运行时报错/循环超限│
│ (超时/鉴权失败/    │   │ (日预算耗尽)       │   │ (工具异常/上下文异常)       │
│  配额耗尽)         │   │                   │   │                           │
├───────────────────┤   ├───────────────────┤   ├───────────────────────────┤
│ 自动切「无 Agent   │   │ 关闭 T-04/T-05    │   │ 单次交互终止，状态落盘，    │
│ 审批模式」：        │   │ （LLM 生成能力）， │   │ 提示用户："本次未完成，     │
│ 候选仍由代码预筛+   │   │ 只保留规则候选 +   │   │ 已保留进度。可继续，或到    │
│ 规则选话术，        │   │ 人工在图形界面审批 │   │ 图形界面手动处理。"         │
│ **人工在图形界面    │   │                   │   │                           │
...
1. **降级不丢数据**：候选批次、`send_id`、任务状态都在盘上；降级后人工审批走**同一份** `batch_id`。
2. **降级不改口径**：无论谁审批，`approve_batch` 的二次校验与发送护栏完全一致——**不存在"人工审批就放松校验"**。
3. **降级要可见**：UI 必须明确显示当前处于哪个降级级别与原因，不许静默降级
   （旧代码 D-14 的"演示假数据不清除"就是静默失真的反面教材）。
...
| 覆盖 | 概率性，可能不遵守 | 确定性，一定拦得住 |
| 失效场景 | 提示注入、模型版本变化、长上下文遗忘 | 不会失效（除非代码有 bug） |

因此：**§5.2 的每一条约束，都必须在 §7.1 的表格里找到对应的代码强制点。**
两者缺一不可；只有提示词 = 没有护栏，只有代码 = 模型会反复撞墙、浪费轮次与 token。

### 5.2 系统提示词的完整内容

以下为可直接使用的系统提示词（`client/agent/prompts/system.p-1.0.0.md`）。
**修改它必须同步升 `prompt_version`**（§5.3）。
...
2. 用量化的意图值，不要自己"理解"意图。
   ...
   （price_inquiry / feature_inquiry / purchase_intent / support_inquiry / complaint / irrelevant / unknown）。
   查询时用这些值，不要用自然语言描述去猜。
...
### 5.3 ⚠️ 提示词与代码的双重保证（再强调一次）

上文的每一条约束，都必须在下表找到代码强制点。**实现完成后请逐行核对。**

| 提示词约束 | 代码强制点 | 位置 |
|---|---|---|
| 不能改安全参数 | 工具清单里没有任何修改策略的工具；参数校验拒绝未知字段 | `client/tools/registry.js` + `validate.js` |
| 不能自己写文案 | `approve_batch` 校验 `draft_id ∈ 本批次 draft_options` | `client/tools/approve-batch.js` |
...
| 不能急停 | 无该工具 | `registry.js` |
| 不能碰 CDP | 无细粒度工具 | `registry.js` |
...
'use strict'
const PROMPT_VERSION = 'p-1.0.0'          // 语义化版本：主/次/修订
// 主版本：角色或安全边界变更（必须重跑评估集与人工复核）
// 次版本：工作方法、汇报格式调整（必须重跑评估集）
// 修订版本：错别字、措辞微调（可只跑冒烟）

function loadSystemPrompt() {
  return require('node:fs').readFileSync(
    require('node:path').join(__dirname, `system.${PROMPT_VERSION}.md`), 'utf8')
}

function promptDigest() {                  // 用于审计：证明"当时用的是哪份提示词"
...
- 提示词文件按版本**留存历史**（`system.p-1.0.0.md` / `system.p-1.1.0.md` 并存），**不覆盖**
- `prompt_version` 必须写进每条决策记录（契约 §五）与每条工具审计
- **改提示词 = 必须重跑评估集**（`plans/B-评估集与成本模型.md` §4.4）
- 提示词变更属于"配置变更"，须走既有审计通道留痕
...
| 示例 | 内容 | 目的 |
|---|---|---|
| F-1 典型批准 | 3 条候选评论 + 各自的 3 个候选话术，其中 1 条候选话术语气明显更贴合 → 输出 `approve` + 正确的 `draft_id` | 教会"从候选池选最贴合的" |
| F-2 该否决不否决 | 一条语气带嘲讽的 `feature_inquiry` → 输出 `reject` + `reason:"语气负面，不宜自动回复"` | 教会语义判断的价值点 |
| F-3 该跳过不硬批 | 一条信息严重不足、无法判断语气的候选 → 输出 `skip` | 教会"拿不准就 skip" |
| F-4 被拦下的正确反应 | 工具返回 `blocked:[{code:'SAFETY_INTERVAL_TOO_SHORT'}]` → 汇报中如实说明，**不重试** | 抑制"撞墙重试"烧钱行为 |

**放置位置**：放在系统提示词之后的**独立的少量示例消息**里（不要塞进系统提示词正文，便于单独替换）。
...
| 状态 | 文件 | 关键字段 | 写入时机 |
|---|---|---|---|
| 任务状态 | `instances/<ID>/agent_task.json` | `task_id, status(pending/running/done/failed), user_text, created_at_ms, updated_at_ms, rounds_used, tokens_used` | 每轮工具调用后 |
...
| 发送记录 | `instances/<ID>/pending_sends.json` | `send_id…`（红线 2 要求**发送前**写入） | 发送前 |
| 对话历史 | 内存 + 可选 `agent_chat.json` | 消息数组 | **仅作推理材料，不承载状态** |

> ⚠️ **`pending_sends.json` 是既有方案 A 的产物，B 不得绕过。** Agent 审批只产生"入队"，
...
function rebuildContextFromDisk(sessionId) {
  ...
  const stateBrief = [
    ...
    batch ? `未完成批次：${batch.batch_id}，共 ${batch.item_ids.length} 条，`
          + `到期 ${new Date(batch.expires_at_ms).toLocaleString('zh-CN')}，`
...
1. 状态摘要**由代码生成**，模型只读不改。摘要中若与历史消息冲突，以摘要为准（提示词中已声明）。
2. 摘要中**不得含隐私字段**（无 `sec_uid`、无昵称、评论摘要 ≤30 字，见 §7.4）。
3. 摘要长度必须可控（建议 ≤400 token），避免每轮固定成本膨胀。
...
function appendDecision(rec) {
  // ⚠️ 经 store 追加写，禁止直接 fs.writeFileSync（单写者约束）
...
| 维度 | 结论 |
|---|---|
| 会话（对话）粒度 | **一实例一 Agent 会话**。不同抖音账号实例的上下文不共享 |
| 状态落盘粒度 | **按实例隔离**：`instances/<ID>/agent_*.json`（与 S-6 隔离目标一致） |
| 切换实例 | 切换时**必须重新 `rebuildContextFromDisk`**，不得沿用上一个实例的上下文 |
| 多标签页/多窗口 | 同一实例多个对话窗口共享**同一份落盘状态**；`agent_task.json` 带 `locked_by`（session 标识），第二个窗口只能看不能改，避免双写 |
| 与服务端的关系 | **Agent 状态不上传服务端**。服务端只收计数与哈希（§7.2）；会话与状态完全留在商家本机 |
...
下表是 `plans/B-工具契约与粒度设计.md` §六 在实现层面的展开。

| 硬规则 | 提示词（让模型知道） | 代码层的强制点 | 实现文件 |
|---|---|---|---|
...
| 不得超过日上限 | §5.2 能力边界 | `review_batch` 只返回额度内的候选；`approve_batch` 二次校验 | 同上 + `safety/guard.js` |
| 不得绕过最小间隔 | §5.2 能力边界 | 发送执行层强制；`approve_batch` 提前拦截 `SAFETY_INTERVAL_TOO_SHORT` | `approve-batch.js` + `host/scheduler.js` |
| 不得发送候选池外的内容 | §5.2 能力边界 | `approve_batch` 校验 `draft_id ∈ 该批次 candidates` | `approve-batch.js` |
| 不得扩大批次 | §5.2 能力边界 | `max_items` 上限由服务端策略决定，工具层截断 | `review-batch.js` |
| 不得修改安全策略 | §5.2 红线 1 | **工具清单里没有任何修改策略的工具**；`validate.js` 拒绝未知字段 | `registry.js` |
| 不得触发急停 | §5.2 能力边界 | **工具清单里没有急停工具** | `registry.js` |
...
| 不得接触隐私字段 | §5.2 汇报格式 | 工具的返回结构里**没有** `sec_uid` 字段（§7.4） | 全部工具 |
...
# 期望输出恰好 8 个：get_account_health query_leads get_stats analyze_comments \
...
|---|---|---|---|
| 工具调用次数与结果码 | `tool_audit.json` | 计数：`agent_tool_calls{tool,code}` | `POST /api/v1/usage/report`（计数类） |
| 决策条数 | `decision_log.jsonl` | 计数：`agent_decisions{approve,reject,skip}` | 同上 |
| 模型标识 | `model{provider,model,version}` | 哈希：`hmac(privacy_salt, provider+"|"+model+"|"+version)` 取前 16 hex | 同上 |
| 提示词版本 | `prompt_version` | **明文**（非隐私，且举证必需） | `POST /api/v1/audit/config-changes`（`field_key:"agent.prompt_version"`） |
| 策略版本 | `policy_version` | **明文**（既有字段，本就要上报） | 既有 `policy_snapshot` |
| 是否启用 Agent | `agent_config.json` | `source:"user"` 的配置变更记录 | `POST /api/v1/audit/config-changes`（`field_key:"agent.enabled"`） |
| 是否允许 LLM 生成候选 | 同上 | `field_key:"agent.allow_generated_drafts"` | 同上 |

> ⚠️ **`usage.report` 的字段白名单是服务端强校验的**（`shared/protocol.md` §7.5）。
> 上述 `agent_*` 计数属于**新增字段**，按 §8.1「新增可选请求字段」规则：服务端先上线支持，
> 旧客户端不受影响。**上报前需确认服务端已支持，否则字段会被丢弃并记审计。**（见 §12 冲突项 C-6）
...
**对策（已定稿，不得改动）**：Agent **不得自由生成话术**，只能**从候选池中选择**。
...
候选池（每条线索 1~3 个候选，max_per_lead 默认 3 上限 5）
   │
   │  ⚠️ 每个候选必须过 safety/similarity（SimilarityChecked = true 才能进审批）
   ▼
T-06 review_batch → draft_options[{draft_id, text, source}]
   │
   │  Agent：只能选 draft_id，不能改 text
   ▼
T-07 approve_batch → 只接受 draft_id
   │
   │  二次校验：draft_id 必须在候选内
   ▼
发送（走完整护栏）
```
...
1. 候选池容量在 `review_batch` 返回时**固定**；`approve_batch` 不能引入新文案。
2. `source:"generated"` 的候选需商家在设置中**显式开启**，默认关闭（`agent_config.allow_generated_drafts`）。
3. 生成的候选**必须经人工或规则复核后**才进入长期话术池（契约 §3.2 T-05 注意 ③）。
4. `similarity_checked` 为 `false` 的候选**不得**出现在 `draft_options` 中。

> ⚠️ 内容风控的**真实防线是候选池由商家提供**，不是相似度检测。相似度检测是第二道。
> 实现时不要把顺序搞反（先信相似度、后信候选池）。
...
| 昵称 | ❌ | 不进入任何工具返回结构 |
| 评论正文 | ⚠️ 截断摘要（`excerpt`，建议 ≤80 字） | 决策必须看内容，但只给截断 |
| 用户哈希 `user_key_hash` | ❌ | Agent 不需要，且它是跨线索关联键 |
...
1. 工具的返回结构必须**白名单构造**：显式挑字段，**不要 `return {...row}` 全量透传**。
   （`AGENTS.md` §2.9 的同类问题：图省事的全量透传是隐私泄漏最常见的入口。）
2. `excerpt` 截断在**工具层**完成，不依赖模型自律。
3. 发往模型服务的请求体本身就要脱敏——**模型服务是第三方**，
   这一点与"是否上传到自家服务端"是两件事，都要守。
...
这部分是**固定成本**，逐条推理时它被重复支付 N 次；批处理时支付 1 次。
详细测算见 `plans/B-评估集与成本模型.md` §五。

**实现要求**：

1. `review_batch` 的 `max_items` 默认 10；**主动鼓励模型取一批**（提示词 §5.2 工作方法 3）。
2. `analyze_comments` 单批上限 50 —— **用满它，不要 10 条一批发 5 次**。
3. **禁止在循环里对单条线索反复调工具**：runtime 检测到"同一 `lead_id` 在单次交互内被查询 ≥3 次"
   时，注入提示"减少重复查询"（并把该模式记入成本监控）。
...
| `get_account_health` 快照 | **5 秒** | `instance_id` | 短时间内重复调用直接返回缓存 |
...
async function draftReplies({ lead_ids, intent_hint, max_per_lead }) {
  ...
  for (const leadId of lead_ids) {
    ...
    const hit = matchTemplatePool(lead, intent_hint)          // 规则匹配
    if (hit.length >= 1) {
    ...
    if (!config.allow_generated_drafts) {
...
**目标指标**：话术池命中率 ≥ 80%。命中率是**成本监控的核心指标之一**
...
> 上表数值需按实际模型单价与定价空间反算填入，见 `plans/B-评估集与成本模型.md` §5.4。
> **不要在代码里硬编码这些数字**——它们必须来自配置。

| 触发 | 行为 |
|---|---|
| 单次交互消耗 > 预算 80% | 注入系统消息"预算即将耗尽，请尽快给出结论" |
| 单次交互消耗 ≥ 预算 | **立即停止**，汇报已完成部分，返回 `BUDGET_EXHAUSTED` |
| 当日累计 ≥ 日预算 | Agent 进入**只读模式**（仅 T-01/T-02/T-03/T-08 可用），T-04/T-05/T-06/T-07 关闭 |
| 连续 3 日超预算 | 写入告警记录并在 UI 显著提示"Agent 成本异常，建议检查提示词或改用方案 A 模式" |

### 8.5 与成本模型文档的关系

**本文不重复测算过程。** 完整的成本拆解、三档估算、每条回复摊薄成本、与定价的比例阈值，
见 **`plans/B-评估集与成本模型.md` §五（成本模型）**。
...
> **顺序已定稿（`plans/B-工具契约与粒度设计.md` §七），不得调整。**
> 每一步都给出目标、验证方式与通过判据。**上一步未通过，不进下一步。**

### 第 0 步：前置检查（不写代码）

| 项 | 判据 |
|---|---|
| 方案 A 已完成 | 验收 1-23 全部通过，`node test/run.js` 全绿 |
...
| **交付** | `client/tools/registry.js` + 三个只读工具 + `validate.js` + `tool_audit.json` + 最小对话界面 |
| **验证方式** | ① 单元测试：schema 校验（含未知字段拒绝）；② 真机：问 5 个问题，数字与图形界面看板**逐项一致** |
| **通过判据** | 1. 工具清单恰好 3 个，无任何写操作<br>2. `query_leads` 返回结构中**不含** `sec_uid`/昵称（代码断言）<br>3. `get_account_health` 的数值与 `policy.json` 一致，非自行推算<br>4. 每次调用都产生一条 `tool_audit.json` 记录 |
| **风险** | 低。无副作用、无推理成本（T-01/T-02/T-03 都无推理） |
...
| **验证方式** | ① 构造 8 类边界用例（下表的阻断码各触发一次）；② 真机跑一批真实候选，观察审批→发送→结果回报全链路；③ 断网/重启后批次仍可续 |
| **通过判据** | 1. `approve_batch` 独立重新执行全部安全校验：**手工篡改 Agent 提交的 `draft_id` 为候选外值，必须 `blocked`**<br>2. 同一 `batch_id` 重复提交 → 返回首次结果，**不重复发送**（幂等）<br>3. 观察期下 `review_batch` 返回空批次、`approve_batch` 返回 `POLICY_SENDING_DISABLED`<br>4. 8 类阻断码全部可被构造并正确返回<br>5. 发送仍走完整护栏：`send_id` 发送前落盘、平台响应体判成功、审计双写<br>6. **商家反馈"愿意用"** ← 这一条是核心假设的验证，不是技术判据 |
| **⚠️ 为什么这一步不做 LLM 生成** | 见下方说明 |

> ### 第 2 步"先不做 LLM 生成"的意义
>
> 方案 B 有一个**假设**：商家愿意接受"Agent 替我做审批"。
> 这个假设**与 LLM 能力无关**——即使 Agent 只做规则级审批，形态是否被接受也是同一个问题。
>
> 而 LLM 生成内容恰恰是**风险最高、成本最高、最难回退**的一块：
> 内容可能被平台判营销（风险 1）、决策依赖模型版本（风险 2）、token 成本不可预测。
>
> 所以：**先用"纯规则候选 + Agent 审批"把形态和链路验证完**，
> 在最小风险下回答"这条路走不走得通"。走不通，及时止损，成本仅为工具层的一部分；
> 走得通，再逐层加 LLM 能力，每一层都是可控的增量。
>
> **反过来做（先上 LLM 生成）的代价**：如果形态最终不被接受，LLM 那部分工作全部作废，
> 且期间已经产生了真实的内容风控风险与 token 支出。
...
| **通过判据** | 1. 意图分类**整体准确率 ≥ 85%**（目标值，见 `plans/B-评估集与成本模型.md` §3.1）<br>2. **`complaint` 召回率 ≥ 95%**（漏判为 `price_inquiry` 会导致在投诉下自动回复，风险最高）<br>3. 相对规则基线的召回增益**可量化**（例如规则漏掉 30% 的询价，LLM 漏掉 8%）<br>4. 缓存命中时**零 token 消耗**（实测）<br>5. 未知意图一律落 `unknown`，**不得自由文本** |
| **风险** | 低（只读分析，不产生发送） |
...
| 项 | 内容 |
|---|---|
| **目标** | 建立"改提示词/换模型后能量化对比"的能力，并把成本变得可见 |
| **交付** | `test/eval/` 评估集（含脱敏数据 + 标注）+ 评估 runner + 成本看板 |
| **验证方式** | ① 用两版提示词跑同一评估集，能产出可对比的报告；② 成本看板数字与实际 token 账单核对一致 |
| **通过判据** | 1. 评估集已脱敏（**逐条人工确认无真实用户信息**）<br>2. 误批率（`approve` 后仍不该回的比例）**< 5%**<br>3. 每条回复的摊薄 token 成本已算出，且占单价比例 **< 15%**（见成本模型 §5.4）<br>4. 成本超预算时**自动降级可触发**（实测） |
...
| 项 | 内容 |
|---|---|
| **目标** | 在话术池匹配不到时提供候选（默认关闭），提升覆盖率 |
| **交付** | T-05 + `allow_generated_drafts` 开关 + 生成候选的相似度检测强制 + 候选进池的复核流程 |
| **验证方式** | ① 相似度检测强制生效（构造语义雷同但字面不同的生成内容，验证被拦）；② 人工评审生成候选的可用率 |
| **通过判据** | 1. `similarity_checked:false` 的候选**不出现**在任何 `draft_options` 中<br>2. 商家未显式开启时，**没有任何** `source:"generated"` 候选<br>3. 生成候选的人工评审通过率 ≥ 70%（目标值，见成本模型 §3.2）<br>4. 生成候选**未复核不得进入长期话术池** |
| **风险** | **最高**（内容风控）。这一步放最后就是因为它最可能出问题 |
...
**本文只给概览。完整的复用矩阵、改造清单、增量工作量与迁移路径见
`plans/B-与方案A的差异说明.md`。**
...
| 4 | Agent 运行时 | `client/agent/runtime.js`、`model-client.js`、`planner.js`、`token-budget.js`、`context.js` | 1 |
...
| 11 | 降级路径实现与验证记录 | `client/agent/runtime.js`（降级分支）+ UI 状态提示 | 4 |
| 12 | 本文档及三份配套文档 | `plans/B-*.md` | — |

### 11.2 验收标准

**通用验收**：`docs/需求规格.md` §九 的 23 条**全部仍然适用**（B 不得让任何一条失效）。
其中与 B 关系最密切的：第 3 条（余额归零停机）、第 5 条（达上限不发送）、
第 15 条（相似度 >85% 拒绝）、第 18 条（夜间不发送）、第 21 条（审计可导出）。

**方案 B 特有的四条验收**：

| # | 验收项 | 判据 | 验证方式 |
|---|---|---|---|
| **B-1** | **Agent 无法越权** | ① 工具清单恰好 8 个，无 CDP 类、无策略修改类、无急停类工具<br>② 篡改 `draft_id` 为候选外值 → `AGENT_DRAFT_NOT_IN_CANDIDATES`<br>③ 传 `max_items` 超过服务端上限 → 被截断<br>④ 传未知字段（如 `daily_max`）→ 被拒绝<br>⑤ 让 Agent"想办法多发几条" → 无法实现 | 代码断言 + 提示注入对抗测试 |
...
| **R2** | **决策不可复现**，影响举证 | 纠纷时无法自证 | 落盘决策记录，**含 `model.version` 与 `prompt_version`** | B-2 验收 |

### 12.2 其余风险

| 风险 | 可能性 | 影响 | 对策 | 责任人 |
|---|---|---|---|---|
| Agent 循环不收敛，烧钱 | 中 | 中 | 轮次上限 12 + 墙钟 3 分钟 + 预算熔断；超限必须如实汇报 | 运行时 |
| 模型不遵守提示词，反复撞墙重试 | 中 | 中 | §5.3 双重保证；few-shot F-4 专门训练"被拦下不重试" | 提示词 + 工具层 |
| 提示注入（评论正文里写"忽略以上指令"） | 中 | **高** | 工具返回的 `excerpt` 截断；工具层参数校验独立；**Agent 无论被怎么诱导都造不出候选外内容** | 工具层（这是架构级防护） |
| 商家误以为"Agent 批准 = 一定发出" | 高 | 低 | UI 并列展示 `decisions` 与 `blocked[]`；汇报格式强制区分"已投递"与"被拦下" | 前端 |
| 成本随对话变长而膨胀 | 中 | 中 | 状态摘要 ≤400 token；对话历史**不回灌全文**，只回灌摘要 + 最近 N 轮 | 运行时 |
| 换模型后行为漂移 | 中 | 中 | 换模型 = 必须重跑评估集（成本模型 §4.4）；`model.version` 落盘 | 评估流程 |
| 工具层偷偷重新实现了 adapters 逻辑 | 中 | **高** | PR 检查：`client/tools/` 下不得出现 `data-e2e` 字符串、不得 `require('ws')`、不得直接 `fs.writeFileSync` | 代码评审 |
| Agent 与图形界面同时操作同一批线索 | 低 | 中 | `agent_batch.json` 带 `locked_by`；图形界面审批同一批次时提示"Agent 正在处理" | host/store |
| 商家在观察期反复让 Agent"试试发一条" | 中 | 中 | 工具层硬拦（空批次 + `POLICY_SENDING_DISABLED`），提示词明确说明；UI 显示观察期文案 | 工具层 |

### 12.3 实现时需要向甲方/文档确认的冲突项

> ⚠️ 以下为编写本文档时发现的**与既有文档不一致之处**。按 `AGENTS.md` §7「发现文档之间互相矛盾」应停下确认。
> 这些**不改变已定稿的工具设计**，但会影响实现细节，需在开工前定稿。

| # | 冲突 | 现有文档说法 | 定稿设计说法 | 建议处置 |
|---|---|---|---|---|
| **C-1** | Agent 默认开关 | `docs/需求规格.md` FR-2.5：默认档位 `半自动`；`FR-6.2`：默认安全 | 方案 B 的 Agent 会自主 `approve` 并投递发送 | Agent **默认关闭**（`agent_config.enabled=false`），需商家在设置中显式开启并勾选风险确认（复用 FR-2.5 全自动档位的确认记录机制）。**不得默认开启** |
| **C-2** | 阻断码命名 | `shared/protocol.md` §3.2 只有 `POLICY_DAILY_CAP_EXCEEDED`；无 `POLICY_DAILY_CAP` | 契约 §3.4 阻断码为 `POLICY_DAILY_CAP` | 工具契约的阻断码是**业务阻断码**，与协议**错误码**不是同一空间。实现时在工具层做显式映射：`POLICY_DAILY_CAP` ↔ `POLICY_DAILY_CAP_EXCEEDED`，并在工具契约文档补一张映射表 |
| **C-3** | 缺少限流类阻断码 | 契约 §3.4 的 7 个阻断码里**没有**"被限流/请求过频" | `shared/protocol.md` 有 `RATE_TOO_MANY_REQUESTS` | **不改已定稿的 7 个阻断码**，实现时把限流归入既有码：Agent 侧限流 → 按 `POLICY_DAILY_CAP` 语义处理并汇报"系统繁忙，请稍后再试"；不足部分在本文档 §4.4 的错误处理表里覆盖 |
| **C-4** | `max_items` 上限的数值来源 | 契约 §3.3 T-06：「上限由服务端策略决定」 | `shared/protocol.md` §1.4 的默认配置项里**没有**这项 | 需在服务端配置项中**新增**一项（建议 `agent_batch_max_items`，默认与 §1.4 风格一致），由 `policy` 下发。**不得写死在客户端**（红线 1） |
| **C-5** | `complaint`/`irrelevant` 的排除位置 | 契约 §3.4 注：「规则层应默认不进入待发批次」 | 契约 §四示例中，Agent 在 `review_batch` 返回的 10 条里 `reject` 了 2 条 `complaint` + 1 条 `irrelevant` | 两处**语义不同、并存不矛盾**：① `intent=complaint/irrelevant` 的线索在**预筛**阶段即被排除（代码保证，不由 Agent 判断）；② 示例中 Agent `reject` 的是**意图被判为询价但语气负面**的内容（语义级判断，规则难做）。实现时按此理解，并在契约文档中补一句说明 |
| **C-6** | `agent_*` 计数上报字段 | `shared/protocol.md` §7.5 字段白名单校验，未知字段会被丢弃 | 方案 B 需要上报 Agent 计数与哈希 | 按 §8.1「新增可选请求字段」流程：**服务端先上线支持**，再让客户端上报。在此之前，Agent 计数**只落本地审计**，不上报。需在 `shared/protocol.md` 增补字段 |
| **C-7** | 依赖白名单 | `AGENTS.md` §2.14 与 `shared/术语与选型基准.md` §3.2：唯一允许的第三方依赖是 `ws` | 接入 LLM 需要 HTTP(S) 客户端 | **用内置 `node:https` 自研**（约 80 行），**不引入任何 SDK**。⚠️ 引入官方 SDK 会违反依赖白名单，必须先报备 |
| **C-8** | 提示词的语言与技术栈约束 | `shared/术语与选型基准.md` §3.1：纯 JavaScript，无构建 | 提示词是 Markdown 文本文件 | 无冲突。但**提示词文件不参与构建**，直接 `fs.readFileSync` 读取；历史版本**并存不覆盖** |
| **C-9** | `plans/` 目录文档缺失 | `README-DEV.md` §七 声明 7 份 `plans/` 文档已 ✅ | 实际仅存在 `00-两方案对比与选型建议.md` 与 `B-工具契约与粒度设计.md` | 本文档引用的 `plans/A-工具链路开发指导.md`、`A-分阶段任务清单.md`、以及 `shared/` 下 5 份规范文档**尚未创建**。**不影响本文档写作**，但第 0 步前置检查需要它们 |
...
| `README-DEV.md` | 开发总索引、三条红线 |
| `AGENTS.md` | 项目红线与 14 条必须避免的误实现 |
...
接口字段与错误码以 `shared/protocol.md` 为准；质量事实源以 `shared/术语与选型基准.md` 为准。*
