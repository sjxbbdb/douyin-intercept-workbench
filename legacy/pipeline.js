...
(async () => {
  const req = JSON.parse(fs.readFileSync("D:\\deep seek\\pipeline_request.json", "utf8"));
  ...
  p.log.push("评论关键词任意字命中: " + commentKeywordChars.join("/"));
  ...
  const matchedVideos = baseVideos.filter((video) => {
  ...
  if (kwOptimized) {
    ...
    p.log.push("关键词匹配：整串匹配 “" + videoKeyword + "”，命中 " + exactHits + " 个");
  }
  saveProgress(p);
  const uniqueVideos = [];
...
// ============ STAGE 3: filter by comment keywords + dedupe + merge ============
  ...
  for (const c of allRaw) {
    ...
    if (!matchesAnyKeywordChar(c.text, commentKeywordChars)) continue;
